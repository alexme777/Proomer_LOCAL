(function($, APP) {
     ...
     * Добавление дизайна
     **/
    APP.Controls.Page.Plan = can.Control.extend({
		...
		init: function () {
			...
			new APP.Controls.DropzoneArea(this.element.find(".js-docs-dropzone"), {
				...
				files: true,
				paramsQuery: {
					fileType: "PROPERTY_ATTACH",
					id_order: this.element.find("#id_order").attr('value'),
				}
			});
		...
		'.js-savestep2 click': function (el, e){
			...
			var action = el.attr('data-next-step');
			var self = this;
			var data = $('.family_composition').serializeArray();
		...
		'.js-savestep3 click': function (el, e){
			...
			var action = el.attr('data-next-step');
			var self = this;
			var data = $('.js-order-settings-form').serializeArray();
		...
		'.js-add-family click': function (el, e){
			...
			this.element.find('#list-people-family').append(html);
			this.element.find("form .js-type-1").select2({
				minimumResultsForSearch: 500,
				width: 'auto'
			});
		...
		'.js-add-family-children click': function (el, e){
			...
			this.element.find('#list-people-children-family').append(html);
			this.element.find("form .js-type-1").select2({
				minimumResultsForSearch: 500,
				width: 'auto'
			});
		...
		'.js-add-family-animal click': function (el, e){
			var html = this.element.find('#people_family_animal').html();
			this.element.find('#list-people-animal-family').append(html);
			this.element.find("form .js-type-1").select2({
				minimumResultsForSearch: 500,
				width: 'auto'
			});
		...
		'.js-remove-family click': function (el, e){
			$('#list-people-family .item-family:last').remove()
		},
		
		'.js-remove-family-children click': function (el, e){
			$('#list-people-children-family .item-family:last').remove()
			//console.log(el.parent().children())	
		},	
		
		'.js-remove-family-animal click': function (el, e){
			$('#list-people-animal-family .item-family:last').remove()
			//console.log(el.parent().children())	
		},
