(function ($, APP) {
    "use strict";
    APP.Controls.DropzoneArea = APP.BaseController.extend(
        {
            defaults: {
                thumbSize: [471, 357],
                url: "",
        ...
        },
        {
                ...
                this.element.dropzone({
                    ...
                    init: function () {
                        ...
                        $dzElement.find('.js-dz-text, .dz-image, .dz-default-img').click(function (ev) {
                            if (!$(ev.target).hasClass("js-del-item") && !$(ev.target).hasClass("js-pin-item")) {
            ...
            appendMultipleImg: function ($dzElement, response) {
                response.forEach(function (val) {
                    ...
                    var $pinIcon = $("<div>", {
                        "class": "js-pin-item pin-icon"
                    });

                    $dzElement.append($newImage.append($delIcon));
                    $dzElement.append($newImage.append($pinIcon));
                })
            },
