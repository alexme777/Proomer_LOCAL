 ...
 */
class PlanController extends Sibirix_Controller {
	...
	public function step1Action() {
		...
		Zend_Registry::get('BX_APPLICATION')->SetPageProperty('page-class', 'create-plan-page');
        Zend_Registry::get('BX_APPLICATION')->SetPageProperty('page-type', 'plan');
		Zend_Registry::get('BX_APPLICATION')->SetTitle(("Выбор планировки"));
		...
		$planoptionModel = new Sibirix_Model_PlanOption();
		$flatModel = new Sibirix_Model_Flat();
		$orderplanModel = new Sibirix_Model_OrderPlan();
		...
		if(Sibirix_Model_User::isAuthorized()){
			...
			====================================*/
			$PROJECT_ARR = $orderplanModel->getOrderList(['=PROPERTY_USER' => $id_user, '=PROPERTY_STATUS' => ORDER_PLAN_STATUS_ADDED], [], 1, false, 1);
		...
		if($this->getParam('selected_plan')){
			...
			$id_flat = $this->getParam('selected_plan');
			$filter['!=ID'] = $id_flat;
			$SELECTED_PLAN = $planModel->getPlanList(['=ID' => $id_flat], $catalogSort, 1, false, 1);
			foreach($SELECTED_PLAN->items as $plan){
				if (!empty($plan->PROPERTY_IMAGES_VALUE)) {
					$image = Resizer::resizeImage($plan->PROPERTY_IMAGES_VALUE, 'COMPLEX_LIST');
				} else {
					$image = '/local/images/proomer2.png';
		...
		$PLAN_ARR = $planModel->getPlanList($filter,$catalogSort, 1, false, $limit);
		...
		$a_result = $orderplanModel->getOrderList(['=ID' => $id_plan], $catalogSort, 1, false, 1);
		...
		$this->view->assign([
			...
			"SELECTED_PLAN"		=> $SELECTED_PLAN->items,
	...
	public function savestep1Action(){
		...
		if(Sibirix_Model_User::isAuthorized()){
			...
			$item = $result->items[0];
				...
				$fields["PROPERTY_VALUES"]["STATUS"] = ORDER_PLAN_STATUS_ADDED;
				$fields["PROPERTY_VALUES"]["ID_FLAT"] = $id_flat;
			
			//проверим есть ли уже добавленные заказы
			$PROJECT_ARR = $orderplanModel->getOrderList(['=PROPERTY_USER' => $id_user, '=PROPERTY_STATUS' => ORDER_PLAN_STATUS_ADDED], $catalogSort, 1, false, 1);
			$count = $PROJECT_ARR->pageData->totalItemsCount;
			if($count == 0){
				//адд
				$fields["PROPERTY_VALUES"]["STEP"] = 2;
				$orderplanModel -> addOrder($fields);
			}
			else{
				//упдате
				...
				$orderplanModel -> editOrder($fields);
			};
		}
		else{
			//$APP = Zend_Registry::get('BX_APPLICATION');
	...
	public function step2Action() {
		...
		$planIds = array_unique($planIds);
		$PLAN_ARR = $planModel->getPlanList(['=ID'=>$planIds],[], 1, false, 999);
	...
	public function savestep2Action(){
		...
		if(Sibirix_Model_User::isAuthorized()){
			...
			//сохраним св-ва комнаты в массиве для bitrix
			$fields = array();
			...
			$fields["PROPERTY_VALUES"]["ANIMAL"] = $defaultValues['animal'];
			//массив с заказом
			$PROJECT_ARR = $orderplanModel->getOrderList(['=PROPERTY_USER' => $id_user, '=PROPERTY_STATUS' => ORDER_PLAN_STATUS_ADDED], [], 1, false, 1);
			...
			if($count == 0){
				...
				CIBlockElement::SetPropertyValuesEx($PROJECT_ARR->items[0]->ID, IB_ORDER_PLAN_OPTION, $fields['PROPERTY_VALUES']);
			};
		}
		else{
	...
	public function step3Action() {
		...
		$a_result = $orderplanModel->getOrderList(['=ID' => $id_plan], $catalogSort, 1, false, 1);
		$starting_price = ($PLAN_SAVE->items[0]->PROPERTY_AREA_VALUE * PRICE_SQUARE) + ($PLAN_SAVE->items[0]->PROPERTY_AREA_VALUE * PRICE_SQUARE) / 100 * COMISSION_PERCENT;
    ...
    public function uploadplanAction() {
        ...
        $fileType   = $this->getParam("fileType");
        $resizeType = $this->getParam("resizeType");
		$id_order = $this->getParam("id_order");
        ...
        switch ($fileType) {
            ...
            case "PROPERTY_PLAN_FLAT":
                $accessExtensions = "jpg,png,jpeg,gif,bmp";
                break;
            case "PROPERTY_ATTACH":
                $accessExtensions = "jpg,png,jpeg,gif,bmp,doc,docx,xls,xlsx,pdf,rar,tar,zip";
                break;
            default:
                $this->_helper->json(['result' => false, "response" => "Invalid file type"]);
                break;
        ...
        $editResult = $orderplanModel->editOrder($fields);
	
        $newFile = false;
        if ($editResult) {
            $newFile = $orderplanModel->select([$fileType], true)->getElement($id_order);
        }

        if ($this->getParam("files") == "true") {
            $response = array();
            if ($editResult) {
                $fileData = array_shift(EHelper::getFileData($newFile->$fileType));
